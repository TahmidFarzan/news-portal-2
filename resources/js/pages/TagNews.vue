...
<template>
    ...
    <Head :title="tag?.name || t('common.labels.tag')">
    ...
    <div class="entity-page tag-page space-y-6">
        <section class="grid grid-cols-1 items-center gap-5 md:grid-cols-12">
            <div class="md:col-span-3 lg:col-span-2">
                <div class="flex h-28 w-28 items-center justify-center rounded-2xl bg-blue-50 p-4 sm:h-32 sm:w-32">
                    <img :src="'/uploads/images/logo/tag.png'" :alt="tag?.name || t('pages.tagNews.tags.details.tagImageAlt')"
        ...
        <GoogleAd v-if="googleAdEnable" :page="adPages.TAG" :type="adTypes.SECTION" :placement="adPlacements.ONE" />

        <div class="border-t border-gray-200"></div>

        <GoogleAd v-if="googleAdEnable" :page="adPages.TAG" :type="adTypes.SECTION" :placement="adPlacements.TWO" />

        <List :news="news" pagination-type="Cursor" />

        <GoogleAd v-if="googleAdEnable" :page="adPages.TAG" :type="adTypes.SECTION" :placement="adPlacements.THREE" />
    </div>

    <Teleport to="body">
        <GoogleAd v-if="googleAdEnable" :page="adPages.TAG" :type="adTypes.POPUP" />
    </Teleport>
</template>
