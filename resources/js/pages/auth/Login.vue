...
function validateForm() {
    ...
    if (!loginForm.email || loginForm.email.trim() === '') {
        loginForm.setError('email', t('common.validation.emailIsRequired'))
        valid = false
    } else if (loginForm.email.length > 200) {
        loginForm.setError('email', t('common.validation.emailMustNotExceed200Characters'))
        valid = false
    }

    if (!loginForm.password || loginForm.password.trim() === '') {
        loginForm.setError('password', t('common.validation.passwordIsRequired'))
        valid = false
    }
...
<template>
    ...
    <Head :title="t('common.labels.login')" />
    ...
    <div class="auth-entry min-h-screen flex items-center justify-center px-4">
        <div class="auth-card w-full max-w-md bg-white shadow rounded-2xl p-6 border border-gray-200">
            ...
            <div class="text-center mb-6">
                <img :src="'/uploads/icons/auth/login.png'" :alt="t('common.labels.login')"
            ...
            <form @submit.prevent="handleLogin">
                ...
                <div class="mb-4">
                    <input id="email" v-model="loginForm.email" type="email"
                        :placeholder="t('auth.login.emailPlaceholder')" autofocus
