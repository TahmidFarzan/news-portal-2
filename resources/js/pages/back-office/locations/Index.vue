...
onMounted(async () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                { text: t('layout_menus.locations'), active: true },
            ],
        })
    )
})
...
<template>
    ...
    <Head :title="t('layout_menus.locations')" />

    <div class="w-full space-y-6">

        <div class="flex justify-between items-center">
            <h2 class="text-lg font-semibold">
                {{ t('layout_menus.locations') }}
            </h2>
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <MultiSelectInfinityLoadingApi :form="filterForm" fieldName="parent_id"
                    ...
                    :multiple="false" :placeholder="t('locations.index.parent_placeholder')" />
                ...
                <MultiSelectInfinityLoadingApi :form="filterForm" fieldName="category_id"
                    ...
                    :multiple="false" :placeholder="t('locations.index.category_placeholder')" />

                <input type="date" v-model="filterForm.date"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />

                <input type="search" v-model="filterForm.search" :placeholder="t('locations.index.search_placeholder')"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            ...
            <div class="flex justify-end">
                <button type="submit" :disabled="filterForm.processing"
                    ...
                    {{ filterForm.processing ? t('locations.index.applying_filter') : t('locations.index.apply_filter')
                    }}
                </button>
            </div>
        </form>
                ...
                <table class="min-w-full text-sm">
                    ...
                    <thead class="bg-gray-50 text-gray-600 text-xs uppercase">
                        <tr>
                            ...
                            <th class="px-4 py-3 text-left">{{ t('locations.index.created') }}</th>
