...
function handleSave() {
    ...
    if (isUpdate.value) {
        inertiaJsRoute.post(
            route('back-office.quizzes.quiz-questions.update', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug, }),
            { ...saveForm.data(), _method: 'patch' },
            requestConfig
        )
    } else {
        saveForm.post(route('back-office.quizzes.quiz-questions.save', { slug: quiz?.slug }), requestConfig)
    }
}
                ...
                <div class="bg-white border rounded-xl p-5 shadow-sm space-y-4">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                        ...
                        <div>
                            ...
                            <SelectInfinityLoadingApi :form="saveForm" fieldName="answer_type" :selectedItem="saveForm.answer_type"
                            :apiUrl="route('search.quiz-question-answer-types')" :multiple="false" :placeholder="t('common.labels.answerType')" />
