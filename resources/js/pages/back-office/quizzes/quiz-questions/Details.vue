    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                ...
                { text: t('common.labels.quizQuestions'), href: route('back-office.quizzes.quiz-questions.index', { slug: quiz?.slug }), active: false },
                {
                    text: `${quizQuestion?.question} ${t('common.actions.details')}`,
                    active: true
                }
            ],
        })
...
<template>
    ...
    <Head :title="`${quizQuestion?.question} ${t('admin.quizQuestions.details.labels.quizQuestion')}`" />

    <div class="w-full space-y-6">

        <div class="flex justify-between items-center">
            <h2 class="text-lg font-semibold">
                {{ t('admin.quizQuestions.details.labels.quizQuestion') }}
            </h2>

            <div class="flex gap-2">
                <a v-if="canUpdate(quizQuestion)"
                    :href="route('back-office.quizzes.quiz-questions.edit', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug })"
                ...
                <a v-if="canAccessQuestionOption()"
                    :href="route('back-office.quizzes.quiz-questions.quiz-question-options.index', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug })"
                ...
                <a v-if="canCreateQuestionOption()"
                    :href="route('back-office.quizzes.quiz-questions.quiz-question-options.create', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug })"
                    class="bg-gray-500 hover:bg-gray-600 text-white px-4 py-2 rounded-md flex items-center gap-2 transition">
                    <FontAwesomeIcon icon="add" />
                    {{ t('common.messages.createQuestionOption') }}
                </a>
            </div>
        </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                <div class="border border-gray-200 rounded-lg p-4 space-y-2">
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">{{ quizQuestion?.question ||
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-4 text-sm">
                        ...
                        <span class="font-medium">
                            {{
                                quizQuestion?.created_at
                                    ? formatDateTime(quizQuestion.created_at)
                ...
                <div class="border border-gray-200 rounded-lg p-4 space-y-2">
                        ...
                        <span class="font-medium">
                            {{
                                quizQuestion?.updated_at
                                    ? formatDateTime(quizQuestion.updated_at)
                                    : t('common.labels.notAvailable')
                            }}
                        </span>
                    </div>
                    ...
                    <div class="flex justify-between">
                        ...
                        <span class="font-medium">
                            {{ quizQuestion?.latest_activity_log?.causer?.name ||
                                t('common.labels.notAvailable') }}
                        </span>
                    </div>
                </div>
        ...
        <div class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            ...
            <RecentActivities :model-slug="'quiz-question'" :model="quizQuestion" />
        </div>
