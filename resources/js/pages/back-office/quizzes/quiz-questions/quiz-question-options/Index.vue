...
const { quiz, quizQuestion, quizQuestionOptions } = defineProps({
    quiz: Object,
    quizQuestion: Object,
    quizQuestionOptions: Object,
})

const paginationOnly = computed(() => {
    if (!quizQuestionOptions) return {}

    const { data, ...rest } = quizQuestionOptions
...
const canCreate = () => canCreateQuizQuestionOption(authUser?.value)
const canUpdate = (quizQuestionOption) => canUpdateQuizQuestionOption(authUser?.value, quizQuestionOption)
const canDelete = (quizQuestionOption) => canDeleteQuizQuestionOption(authUser?.value, quizQuestionOption)
...
const handleDelete = (quizQuestionOption) => {
    ...
    inertiaJsRoute.delete(route('back-office.quizzes.quiz-questions.quiz-question-options.delete', { slug: quiz?.slug, quizQuestionSlug:quizQuestion?.slug, quizQuestionOptionSlug: quizQuestionOption?.slug }), {
        onFinish: () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                ...
                { text: t('common.labels.quizQuestionOptions'), active: true },
            ],
        })
    )
})
...
<template>
    ...
    <Head :title="t('common.labels.quizQuestionOptions')" />
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a v-if="canCreate()" :href="route('back-office.quizzes.quiz-questions.quiz-question-options.create', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug })"
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input v-model="filterForm.search" type="search"
                    :placeholder="t('common.placeholders.searchQuizQuestionOption')"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in quizQuestionOptions?.data" :key="item.id"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.quizzes.quiz-questions.quiz-question-options.details', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug, quizQuestionOptionSlug: item?.slug })"
                                    ...
                                    <a v-if="canUpdate(item)"
                                        :href="route('back-office.quizzes.quiz-questions.quiz-question-options.edit', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug, quizQuestionOptionSlug: item?.slug })"
                        ...
                        <tr v-if="!quizQuestionOptions?.data?.length">
