...
const { quiz, quizQuestions } = defineProps({
    quiz: Object,
...
const applyFilter = () => {
    ...
    inertiaJsRoute.get(route('back-office.quiz-questions.index'), cleanParams, {
        replace: true,
        preserveScroll: true,
        preserveState: true,
    })
...
const confirmDelete = (quizQuestion) => {
    deletingRow.value = quizQuestion
    showDeleteModal.value = true
}

const canCreate = () => canCreateQuizQuestion(authUser?.value)
const canUpdate = (quizQuestion) => canUpdateQuizQuestion(authUser?.value, quizQuestion)
const canDelete = (quizQuestion) => canDeleteQuizQuestion(authUser?.value, quizQuestion)
...
const handleDelete = (quizQuestion) => {
    if (!quizQuestion || deleteProcessing.value) return

    deleteProcessing.value = true

    inertiaJsRoute.delete(route('back-office.quizzes.quiz-questions.delete', { slug: quiz?.slug, quizQuestionSlug: quizQuestion?.slug }), {
        onFinish: () => {
    ...
    window.dispatchEvent(
        new CustomEvent('set-breadcrumb', {
            detail: [
                ...
                { text: t('common.labels.quizQuestions'), active: true },
            ],
        })
    )
})
...
<template>
    ...
    <Head :title="t('common.labels.quizQuestions')" />
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a v-if="canCreate()" :href="route('back-office.quizzes.quiz-questions.create', { slug: quiz.slug })"
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input v-model="filterForm.search" type="search" :placeholder="t('common.placeholders.searchQuizQuestion')"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in quizQuestions?.data" :key="item.id"
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.quizzes.quiz-questions.details', { slug: quiz?.slug, quizQuestionSlug: item.slug })"
                                    ...
                                    <a v-if="canUpdate(item)"
                                        :href="route('back-office.quizzes.quiz-questions.edit', { slug: quiz?.slug, quizQuestionSlug: item?.slug })"
                                    ...
                                    <a v-if="canAccessQuestionOption()"
                                        :href="route('back-office.quizzes.quiz-questions.quiz-question-options.index', { slug: quiz?.slug, quizQuestionSlug: item.slug })"
                                        class="p-2 rounded-md text-blue-500 hover:bg-blue-50 border"
                                        :title="t('common.messages.questionOptions')">
                                        <FontAwesomeIcon icon="list" />
                                    </a>

                                    <a v-if="canCreateQuestionOption()"
                                        :href="route('back-office.quizzes.quiz-questions.quiz-question-options.create', { slug: quiz?.slug, quizQuestionSlug: item.slug })"
                                        class="p-2 rounded-md text-green-600 hover:bg-green-50 border"
                                        :title="t('common.messages.createQuestionOption')">
                        ...
                        <tr v-if="!quizQuestions?.data?.length">
