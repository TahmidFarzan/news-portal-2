...
const applyFilter = () => {
    ...
    intertiaJsRoute.get(
        route('back-office.quizzes.index'),
        cleanParams,
        {
            replace: true,
            preserveScroll: true,
...
const confirmStatus = (quiz, action) => {
    statusRow.value = quiz
...
const handleDelete = (quiz) => {
    if (!quiz || deleteProcessing.value) return

    deleteProcessing.value = true

    intertiaJsRoute.delete(
        route(
            'back-office.quizzes.delete',
            {
                slug: quiz.slug
            }
        ),
...
const executeStatusAction = () => {
    ...
    intertiaJsRoute.patch(
        route(
            isInactive
                ? 'back-office.quizzes.inactive'
                : 'back-office.quizzes.active',
            { slug }
        ),
        {},
        {
    ...
    window.dispatchEvent(
        new CustomEvent(
            'set-breadcrumb',
            {
                detail: [
                    {
                        text: t('common.labels.quizzes'), active: true
                    }
                ]
            }
        )
...
<template>
    ...
    <Head :title="t('common.labels.quizzes')" />
    ...
    <div class="w-full space-y-6">
        ...
        <div class="flex justify-between items-center">
            ...
            <a :href="route('back-office.quizzes.create')"
        ...
        <form @submit.prevent="applyFilter" class="bg-white border border-gray-200 rounded-xl shadow-sm p-5 space-y-4">
            <div class="grid grid-cols-1 md:grid-cols-4 gap-4">
                ...
                <input type="search" v-model="filterForm.search"
                    :placeholder="t('common.placeholders.searchQuiz')"
                    class="w-full border border-gray-200 rounded-md px-3 py-2 text-sm focus:ring-2 focus:ring-blue-500 focus:outline-none" />
            ...
            <div class="overflow-x-auto">
                <table class="min-w-full text-sm">
                    ...
                    <tbody class="divide-y">
                        <tr v-for="(item, index) in quizzes?.data" :key="item.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-4 py-3 text-right">
                                <div class="flex justify-end gap-2">
                                    ...
                                    <a :href="route('back-office.quizzes.details', { slug: item.slug })"
                                    ...
                                    <a v-if="canUpdate(item)"
                                        :href="route('back-office.quizzes.edit', { slug: item.slug })"
                                    ...
                                    <a v-if="canAccessQuestion(item)"
                                        :href="route('back-office.quizzes.quiz-questions.index', { slug: item.slug })"
                                    ...
                                    <a v-if="canCreateQuestion(item)"
                                        :href="route('back-office.quizzes.quiz-questions.create', { slug: item.slug })"
                        ...
                        <tr v-if="!quizzes?.data?.length">
                            <td colspan="6" class="px-4 py-8 text-center text-gray-500">
                                {{ t('admin.quizzes.index.labels.noQuizFound') }}
                            </td>
                        </tr>
                    </tbody>
