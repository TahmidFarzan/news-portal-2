...
<template>
    ...
    >
        ...
        >
            ...
            <div
                v-if="!hideNewsType && hasNewsTypeIcons"
                class="absolute flex flex-wrap"
                :class="isCompact ? 'left-1 top-1 gap-1' : 'left-1 top-1 gap-1 sm:left-2 sm:top-2 sm:gap-1.5'"
            >
                <span
                    v-for="item in newsTypeIcons"
                    :key="item.key"
                    :title="item.title"
        ...
        <div
            class="pointer-events-none relative z-20 min-w-0 flex flex-1 flex-col justify-center"
            :class="isCompact ? 'space-y-1 px-1 py-0.5' : 'space-y-1 sm:space-y-2'"
        >
            <div
                v-if="(news?.category && !hideCategory) || (news?.event && !hideEvent) || (news?.location && !hideLocation)"
            ...
            >
                <a
                    v-if="news?.category && !hideCategory"
                    :href="news.category.public_url"
                    title="Category"
                ...
                <span
                    v-if="news?.event && !hideEvent && news?.category && !hideCategory"
                    class="text-gray-300"
                    aria-hidden="true"
                >
                    |
                ...
                <a
                    v-if="news?.event && !hideEvent"
                    :href="news?.event?.public_url"
                    title="Event"
                    class="inline-flex min-w-0 items-center gap-1 transition duration-300 hover:text-red-600"
                    @click.stop
                >
                ...
                <a
                    v-if="news?.location && !hideLocation"
                    :href="news.location.public_url"
                    title="Location"
                    class="inline-flex min-w-0 items-center gap-1 transition duration-300 hover:text-red-600"
                    @click.stop
                >
            ...
            <p
                v-if="news?.sub_title && !hideSubtitle"
                class="break-words text-gray-600"
                :class="[
            ...
            <h3
                class="break-words font-bold leading-snug text-gray-950 transition duration-300 group-hover:text-red-600"
                :class="[
            ...
            >
                <b
                    v-if="news?.content_shoulder"
                    class="mr-1 font-semibold text-red-600"
            ...
            <div
                class="flex flex-wrap items-center text-gray-500"
            ...
            >
                <div
                    v-if="!shouldShowFeatureImage && !hideNewsType && hasNewsTypeIcons"
                    class="inline-flex items-center"
                    :class="isCompact ? 'gap-1' : 'gap-1 sm:gap-1.5'"
                >
                    <span
                        v-for="item in newsTypeIcons"
                        :key="item.key"
