...
const linkClasses = computed(() => {
    if (isRoot.value) {
        return 'h-8 flex items-center gap-1 text-sm text-gray-300 hover:text-white transition'
    }
...
const dropdownClasses = computed(() => {
    if (isRoot.value) {
        return 'absolute left-0 top-full mt-2 min-w-44 bg-white border border-gray-200 shadow-lg rounded-xl py-1 z-[999]'
    }
    ...
    <li class="relative flex-shrink-0" @mouseenter="isOpen = true" @mouseleave="isOpen = false">
        ...
        <Transition enter-active-class="transition ease-out duration-150"
            ...
            <ul v-if="hasChildren && isOpen" :class="dropdownClasses">
                <TopbarMenuItem v-for="child in children" :key="child.id" :item="child" :level="level + 1" :currentLanguage="currentLanguage" />
            </ul>
        </Transition>
    </li>
</template>
