...
<template>
    <li ref="rootRef" class="relative flex-shrink-0" :class="{ 'w-full': level > 0 }" @mouseenter="openMenu"
        ...
        <Transition enter-active-class="transition ease-out duration-150"
            ...
            <div v-if="item.has_descendants && isOpen" ref="submenuRef" @mouseenter="keepMenuOpen"
                @mouseleave="closeMenu" @scroll.passive="handleSubMenuScroll"
                class="bg-white text-gray-800 shadow-lg border border-gray-200 rounded-xl max-h-72 overflow-y-auto z-[999] [scrollbar-width:thin] [&::-webkit-scrollbar]:w-1.5 [&::-webkit-scrollbar-track]:bg-transparent [&::-webkit-scrollbar-thumb]:rounded-full [&::-webkit-scrollbar-thumb]:bg-gray-300"
                :class="level === 0 ? '' : 'absolute left-full top-0 ml-1 min-w-52'"
                :style="level === 0 ? dropdownStyle : {}">
                <ul class="py-1">
