...
const load = async (force = false) => {
    ...
    try {
        ...
        const cacheOptions = force
            ? { key: `${apiCacheKey.API_HOME_PAGE}:${apiUrl}:force:${Date.now()}`, ttl: 0 }
            : {
                key: `${apiCacheKey.API_HOME_PAGE}:${apiUrl}`,
                ttl: apiCacheTTL.HOME_PAGE,
            }
        const response = await fetchFromApi(apiUrl, {}, cacheOptions)
...
onMounted(() => load())
watch(
    () => [currentLanguage?.code, currentLanguage?.is_default],
    () => load()
)
</script>
<template>
